...
<div class="container-fluid">
   ...
   <form class="text-center border border-light p-5" method="post" enctype="multipart/form-data">
      ...
      <p class="h2 mb-4 text-success">Authorize Supplier</p>
      <hr>
      ...
      <span class="btn btn-sm btn-success " onclick="event.preventDefault(); document.getElementById('approve-form').submit();">Authorize</span>
      <span class="btn btn-sm btn-danger"  onclick="event.preventDefault(); document.getElementById('reject-form').submit();">Reject</span>
      <div>
         <p><a href="{{route('todo.supplierdoc',$todos->id)}}" target="_blank">{{$todos->file}}</a></p>
      </div>
   </form>

   <form id="approve-form" action="{{route('todo.updateSupplier',$todos->id)}}" method="POST" style="display: none;">
      @csrf
      @method('patch')
   </form>

   <form id="reject-form" action="{{route('todo.rejectSupplier',$todos->id)}}" method="POST" style="display: none;">
      @csrf
      @method('patch')
   </form>
</div>
